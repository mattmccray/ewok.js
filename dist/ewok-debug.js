...
(function() {
  var FieldError, FieldErrorCollection, FormModel, getValueFromField, refreshJqueryMobile, setValueForField,
    __bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; },
    __hasProp = {}.hasOwnProperty;

  getValueFromField = function(el, dom) {
    switch (dom.type) {
      case 'radio':
        return el.find("[name=" + dom.name + "]:checked").val();
      case 'checkbox':
  ...
  setValueForField = function(el, field, value) {
    var checkIt, dom;
    ...
    if (dom) {
      switch (dom.type) {
        case 'radio':
          ...
          return refreshJqueryMobile(el.find("[name=" + field + "]"));
        case 'checkbox':
          checkIt = value === true || value === 'on' || value === 'true' || value === '1' || value > 0 || value === dom.value;
          $(dom).attr('checked', checkIt);
          return refreshJqueryMobile($(dom));
        default:
          $(dom).val(value);
          return refreshJqueryMobile($(dom));
      }
  ...
  refreshJqueryMobile = function(el) {
    if ($.mobile) {
      try {
        if (el.is(":checkbox,:radio")) {
          return el.checkboxradio('refresh');
        } else if (el.is("select")) {
          return el.selectmenu('refresh');
        } else if (el.is("input[type='range']")) {
          return el.slider("refresh");
        } else if (el.is('input')) {
          return el.textinput('refresh');
        }
      } catch (_error) {}
    }
  };
  ...
  FieldError = (function() {
    ...
    function FieldError(name, elem) {
      this.name = name;
      this.elem = elem;
      this.displayName = _.humanize(this.elem.attr('placeholder') || this.elem.data('display-name') || this.elem.parents('form').find("label[for=" + (this.elem.attr('id')) + "]").text() || this.name);
      this.messages = [];
    }

    FieldError.prototype.addMessage = function(msg) {
      this.messages.push(msg);
      return this;
    };

    FieldError.prototype.toString = function() {
      return "" + this.displayName + " " + (this.messages.join(' and ')) + ".";
    };
  ...
  FieldErrorCollection = (function() {

    function FieldErrorCollection() {
      this.reset();
    }

    FieldErrorCollection.prototype.reset = function() {
      this.errors = {};
      this.length = 0;
      this.first = null;
      return this;
    ...
    FieldErrorCollection.prototype.addError = function(field, el, msg) {
      var _base;
      (_base = this.errors)[field] || (_base[field] = new FieldError(field, el));
      this.errors[field].addMessage(msg);
      this.length = _.keys(this.errors).length;
      if (!this.first) {
        this.first = el;
      }
      return this;
    ...
    FieldErrorCollection.prototype.focusFirstError = function() {
      var _ref;
      if (this.first) {
        return (_ref = this.first.get(0)) != null ? _ref.focus() : void 0;
      }
    };

    FieldErrorCollection.prototype.isEmpty = function() {
      return _.isEmpty(this.errors);
    };

    FieldErrorCollection.prototype.each = function(fn) {
      return _.each(this.errors, fn);
    };

    FieldErrorCollection.prototype.map = function(fn) {
      return _.map(this.errors, fn);
    };

    FieldErrorCollection.prototype.toString = function() {
      return "" + (this.map(function(e) {
        return e.toString();
      }).join(' '));
    };

    return FieldErrorCollection;

  })();

  FormModel = (function() {

    function FormModel(el, validations) {
      var _this = this;
      this.el = el;
      this.gather = __bind(this.gather, this);

      this.model = new Backbone.Model;
      this.errors = new FieldErrorCollection;
      this.gather();
      this.originalProps = this.model.toJSON();
      this._validationRules = {};
      this.onSubmitCallback = function() {};
      this.el.bind('input', function(event) {
        return _this.gatherOne(event.target);
      });
      this.el.bind('change', function(event) {
        return _this.gatherOne(event.target);
      });
      this.el.bind('submit', function(event) {
        _this.gather();
        return _this.onSubmitCallback(event);
      });
      if (this.validations) {
    ...
    FormModel.prototype.setValidations = function(rules, reset) {
      var field, rule, ruleset, _base, _i, _len, _ref;
      if (reset == null) {
        reset = false;
      }
      if (reset) {
      ...
      for (field in rules) {
        if (!__hasProp.call(rules, field)) continue;
        ruleset = rules[field];
        (_base = this._validationRules)[field] || (_base[field] = []);
        _ref = ruleset.split(',');
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          rule = _ref[_i];
          this._validationRules[field].push(_.trim(rule));
        }
      }
      return this;
    };
    ...
    FormModel.prototype.validate = function() {
      var err, field, rule, ruleset, _i, _len, _ref;
      this.errors.reset();
      _ref = this._validationRules;
      for (field in _ref) {
        ...
        for (_i = 0, _len = ruleset.length; _i < _len; _i++) {
          rule = ruleset[_i];
          if (FormModel.Validator.test(rule, this.model.get(field)) === false) {
            this.errors.addError(field, this.getElemForField(field), FormModel.Validator.Description["for"](rule));
          }
        }
      }
      return this;
    ...
    FormModel.prototype.isValid = function() {
      this.validate();
      return this.hasErrors();
    };

    FormModel.prototype.hasErrors = function() {
      return this.errors.isEmpty();
    };

    FormModel.prototype.getElemForField = function(field) {
      var elem, _ref;
      elem = this.el.find(":input[name=" + field + "]");
      if (((_ref = elem.get(0)) != null ? _ref.type : void 0) === 'radio') {
        return this.el.find(":input[name=" + field + "][value=" + (this.get(field)) + "]");
      } else {
        return elem;
      }
    };

    FormModel.prototype.numChanges = function(gather_first) {
      var key, mismatches, value, _ref;
      if (gather_first == null) {
        gather_first = false;
      }
      if (gather_first) {
        this.gather();
      ...
      mismatches = 0;
      _ref = this.originalProps;
      for (key in _ref) {
        if (!__hasProp.call(_ref, key)) continue;
        value = _ref[key];
        if ((this.model.get(key) || "").toString() !== (value || "").toString()) {
          mismatches += 1;
        }
      }
      return mismatches;
    ...
    FormModel.prototype.isDirty = function() {
      var mismatches;
      mismatches = this.numChanges(true);
      return mismatches !== 0;
    };

    FormModel.prototype.changedFields = function() {
      var key, mismatches, value, _ref;
      this.gather();
      mismatches = [];
      _ref = this.originalProps;
    ...
    FormModel.prototype.populate = function(obj, reset_dirty_flag) {
      var data;
      if (reset_dirty_flag == null) {
        reset_dirty_flag = true;
      }
      data = obj.toJSON ? obj.toJSON() : obj;
      if (reset_dirty_flag) {
        this.originalProps = _.clone(data);
      }
      this.set(_.clone(data));
      return this;
    };

    FormModel.prototype.markAsClean = function() {
      this.gather();
      this.originalProps = this.toJSON();
      return this;
    };

    FormModel.prototype.reset = function(data) {
      var _ref;
      if (data == null) {
        data = {};
      }
      if ((_ref = this.el.get(0)) != null) {
        _ref.reset();
      }
      this.set(_.clone(data));
      return this;
    ...
    FormModel.prototype.gather = function() {
      var _this = this;
      this.el.find(':input').each(function(idx, dom) {
        return _this.gatherOne(dom);
      });
      return this;
    };

    FormModel.prototype.gatherOne = function(dom) {
      var val;
      if (dom.name) {
        val = getValueFromField(this.el, dom);
        this.model.set(dom.name, val);
      }
      return this;
    };

    FormModel.prototype.focus = function(field) {
      var _ref;
      return (_ref = this.getElemForField(field).get(0)) != null ? _ref.focus() : void 0;
    };

    FormModel.prototype.enable = function(sel, updateJqmStyles) {
      if (sel == null) {
        sel = ':input';
      }
      if (updateJqmStyles == null) {
        updateJqmStyles = true;
      }
      this.el.find(sel).removeAttr('disabled');
      if (updateJqmStyles) {
        refreshJqueryMobile(this.el.find(sel));
      }
      return this;
    };

    FormModel.prototype.disable = function(sel, updateJqmStyles) {
      if (sel == null) {
        sel = ':input';
      }
      if (updateJqmStyles == null) {
      ...
      this.el.find(sel).attr('disabled', true);
      if (updateJqmStyles) {
        refreshJqueryMobile(this.el.find(sel));
      }
      return this;
    ...
    FormModel.prototype.enableField = function(field) {
      return this.enable("[name=" + field + "]");
    };

    FormModel.prototype.disableField = function(field) {
      return this.disable("[name=" + field + "]");
    };

    FormModel.prototype.enableFields = function() {
    ...
    FormModel.prototype.disableFields = function() {
      var field, fields, _i, _len;
    ...
    FormModel.prototype.get = function(key) {
      return this.model.get(key);
    };

    FormModel.prototype.set = function(atts) {
      var field, value;
      this.internal_events = true;
      for (field in atts) {
        if (!__hasProp.call(atts, field)) continue;
        value = atts[field];
        setValueForField(this.el, field, value);
      }
      this.model.set(atts);
      this.internal_events = false;
      return this;
    };

    FormModel.prototype.getAsNumber = function(key) {
      return Number(this.get(key));
    };

    FormModel.prototype.getAsInt = function(key) {
      return parseInt(this.get(key));
    };

    FormModel.prototype.getAsFloat = function(key) {
      return parseFloat(this.get(key));
    };

    FormModel.prototype.bind = function(event, handler) {
      if (event.toLowerCase() === 'submit') {
        this.onSubmitCallback = handler;
      } else {
        this.model.bind(event, handler);
      }
      return this;
    };

    FormModel.prototype.on = function(e, h) {
      return this.bind(e, h);
    };

    FormModel.prototype.refreshStyles = function() {
      this.el.find(':input').each(function() {});
      return refreshJqueryMobile($(this));
    };

    FormModel.prototype.toJSON = function(excluding) {
      var attrs, field, value, _ref;
      if (excluding == null) {
        excluding = [];
      }
      attrs = {};
    ...
    FormModel.createFrom = function(sel, validations) {
      return new FormModel($(sel), validations);
    };

    FormModel.create = function(sel, validations) {
      return this.createFrom(sel, validations);
    };

    FormModel.Validator = {
      define: function(rule, description, fn) {
      ...
      Description: {
        "for": function(rule) {
          return FormModel.Validator.Description[rule] || FormModel.Validator.Description.unknown;
        },
  ...
  FormModel.Validator.define('required', 'is required', function(value) {
    return value !== null && value !== "" && !_.isUndefined(value);
  });

  if ($) {
    $.fn.formModel = function(validations) {
      var formModel;
      if (!(formModel = this.data('formModel'))) {
        formModel = new FormModel(this, validations);
